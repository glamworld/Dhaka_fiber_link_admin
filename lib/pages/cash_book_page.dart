...
class _CashBookState extends State<CashBook> with SingleTickerProviderStateMixin{
  ...
  Widget build(BuildContext context) {
    ...
    if (_counter == 0) {
      setState(() {
        headList=headProvider.cashBookList;
    ...
    setState(() {
      headAccounts=headProvider.headOfAccountCashList;
    });
    return Container(
        ...
        children: [
          ...
          _isLoading
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context,index)=>CashBookTableBody(
    ...
    if(auth.headModel.name!.isNotEmpty && auth.headModel.details!.isNotEmpty && auth.headModel.debit!.isNotEmpty && auth.headModel.credit!.isNotEmpty &&
      ...
      if(snapShot.exists){
        ...
        Future.delayed(Duration(seconds: 2), ()async {
          bool result = await auth.addCashBookDetails2(auth.headModel,auth.currentExpenseList[0].totalCost);
          if(result){
            setState(() {
              _isLoading=false;
            });
        ...
        Future.delayed(Duration(seconds: 2), ()async {
          bool result = await auth.addCashBookDetails(auth.headModel);
          if(result){
            setState(() {
              _isLoading=false;
            });
                        ...
                        RaisedButton(
                          ...
                          onPressed: () {
                            ...
                            if (_addKey.currentState!.validate()) {
                              ...
                              setState(() {
                                ap.name = name;
                                headProvider.addHeadOfAccountCash(ap);
                              });
                              //Navigator.of(context).pop();
                            }
                          },
